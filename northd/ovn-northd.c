...
build_pre_acl_flows(struct ovn_datapath *od, struct ovn_port *op,
                    struct hmap *lflows, bool has_stateful)
{
    if (has_stateful) {
    ...
    if (!strcmp(op->nbsp->type, "localnet")) {
        struct ds match_localnet_in = DS_EMPTY_INITIALIZER;
        struct ds match_localnet_out = DS_EMPTY_INITIALIZER;
        ds_put_format(&match_localnet_in, "ip && inport == %s && "
                        "(nd_rs || nd_ra)", op->json_key);
        ds_put_format(&match_localnet_out, "ip && outport == %s && " 
                        "(nd_rs || nd_ra)", op->json_key);
        ovn_lflow_add_with_hint(lflows, od, S_SWITCH_IN_PRE_ACL, INT16_MAX,
                                ds_cstr(&match_localnet_in), "drop;",
                                &op->nbsp->header_);
        ovn_lflow_add_with_hint(lflows, od, S_SWITCH_OUT_PRE_ACL, INT16_MAX,
                                ds_cstr(&match_localnet_out), "drop;",
                                &op->nbsp->header_);

        ds_destroy(&match_localnet_in);
        ds_destroy(&match_localnet_out);
    }
}
               ...
               struct hmap *lflows)
{
    ...
    for (size_t i = 0; i < od->n_router_ports; i++) {
        build_pre_acl_flows(od, od->router_ports[i], lflows, has_stateful);
    }
    for (size_t i = 0; i < od->n_localnet_ports; i++) {
        build_pre_acl_flows(od, od->localnet_ports[i], lflows, has_stateful);
    }
    if (has_stateful) {
